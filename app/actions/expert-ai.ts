...
// 전문가 AI 응답 생성 함수
export async function generateExpertResponse(question: string, category: CategoryType = "all") {
  try {
    // 카테고리별 전문가 페르소나 설정
    ...
    switch (category) {
      case "development":
        ...
        additionalContext = "아이의 신체, 인지, 언어, 사회성 발달에 관한 전문 지식을 바탕으로 답변해주세요."
        break
      case "sleep":
        expertPersona = "소아 수면 전문가"
        additionalContext = "아이의 수면 패턴, 수면 습관 형성, 수면 문제 해결에 관한 전문 지식을 바탕으로 답변해주세요."
        break
      case "nutrition":
        expertPersona = "아동 영양 전문가"
        additionalContext = "아이의 영양 섭취, 식습관 형성, 이유식, 편식 문제에 관한 전문 지식을 바탕으로 답변해주세요."
        break
      case "behavior":
        expertPersona = "아동 행동 전문가"
        additionalContext = "아이의 행동 발달, 훈육 방법, 문제 행동 대처에 관한 전문 지식을 바탕으로 답변해주세요."
        break
      case "psychology":
        expertPersona = "아동 심리학자"
        additionalContext = "아이의 정서 발달, 심리적 안정, 애착 형성에 관한 전문 지식을 바탕으로 답변해주세요."
        break
      case "education":
        expertPersona = "유아 교육 전문가"
        additionalContext = "아이의 학습 발달, 교육 방법, 학습 환경 조성에 관한 전문 지식을 바탕으로 답변해주세요."
        break
    }
    ...
    return {
      content: "죄송합니다. 현재 전문가 AI 서비스에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해 주세요.",
      role: "assistant",
      createdAt: new Date(),
    }
  }
