...
export default function ExpertProfilePage({ params }: { params: { id: string } }) {
  ...
  const expert = {
    ...
    education: ["서울대학교 의과대학 의학박사", "서울대학교병원 소아과 전문의", "미국 스탠포드 대학교 수면의학 연수"],
    experience: [
      "현) 서울대학교 의과대학 소아과 교수",
      "현) 대한수면의학회 이사",
          ...
          <Card>
            ...
            <CardFooter className="flex flex-col gap-2">
              <Button className="w-full" asChild>
                <Link href={`/expert/consultation/${expert.id}`}>상담 예약하기</Link>
              </Button>
          ...
          <Card className="mt-6">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">{expert.consultationFee}</p>
              </div>
              <div className="space-y-1">
                <p className="text-sm font-medium">상담 가능 시간</p>
                <p className="text-sm text-muted-foreground">{expert.availableTime}</p>
              </div>
            </CardContent>
          </Card>
        </div>
            ...
            <TabsContent value="articles" className="mt-6">
              <div className="space-y-6">
                {expert.articles.map((article) => (
                  <Card key={article.id}>
                    <CardHeader>
                      <Link href={`/expert/advice/${article.id}`} className="hover:underline">
              ...
              {expert.upcomingLives.length > 0 ? (
                <div className="space-y-6">
                  {expert.upcomingLives.map((live) => (
                    <Card key={live.id}>
                      ...
                      <CardFooter>
                        <Button className="w-full" asChild>
                          <Link href={`/expert/live/${live.id}`}>알림 신청하기</Link>
                        </Button>
                      </CardFooter>
                    </Card>
                  ))}
                ...
                <Card>
                  <CardContent className="py-8 text-center">
                    <p className="text-muted-foreground mb-4">현재 예정된 라이브 세션이 없습니다.</p>
                    <Button variant="outline" asChild>
                      <Link href="/expert/live">다른 전문가의 라이브 세션 보기</Link>
                    </Button>
                  </CardContent>
                </Card>
              )}
