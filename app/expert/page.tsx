...
import { Send, RefreshCw, Brain, MessageCircle, Clock, Users, Menu, X } from 'lucide-react';
...
export default function ExpertPage() {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        setError(null);
        const session = await websocketService.createSession('ai_expert', 'general');
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)]">
      {/* Sidebar */}
      <div className={`${isSidebarOpen ? 'fixed inset-0 z-50 lg:static lg:inset-auto' : 'hidden'} lg:block w-full lg:w-80 border-r`}>
        {/* 모바일에서 배경 클릭 시 사이드바 닫기 */}
        {isSidebarOpen && (
          <div className="lg:hidden absolute inset-0 z-40" onClick={() => setIsSidebarOpen(false)} />
        )}
            ...
            <>
              {messages.map((message) => (
                <div key={message.id} className={`flex ${message.isAI ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[70%] ${message.isAI ? 'flex items-start space-x-3' : ''}`}>
                    {message.isAI && (
          ...
          <div className="flex items-end space-x-3">
            <div className="flex-1">
              <textarea
                ...
                placeholder={isConnected ? "질문을 기다리는 중..." : "연결 중... 잠시만 기다려주세요."}
