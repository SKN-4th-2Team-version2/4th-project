...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, account, user }) {
      ...
      if (account && user) {
        try {
          ...
          if (response.data.success) {
            ...
            throw new Error(response.data.message || '소셜 로그인 처리 중 오류가 발생했습니다.');
          }
        } catch (error) {
      ...
      // 토큰 갱신이 필요한 경우
      if (typedToken.djangoRefreshToken && typedToken.exp && typedToken.exp < Math.floor(Date.now() / 1000) + 300) {
        try {
          const response = await axios.post(
