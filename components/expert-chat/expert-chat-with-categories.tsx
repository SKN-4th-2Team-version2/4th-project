...
export type CategoryType = "all" | "development" | "sleep" | "nutrition" | "behavior" | "psychology" | "education"
...
export function ExpertChatWithCategories({ initialCategory = "all", continueFromId }: ExpertChatWithCategoriesProps) {
  ...
  useEffect(() => {
    ...
    let welcomeMessage = "안녕하세요! 마파덜의 육아 전문가 AI입니다. 어떤 질문이든 편하게 물어보세요."
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "죄송합니다. 응답을 생성하는 중에 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.",
          createdAt: new Date(),
        },
      ])
    } finally {
  ...
  const handleSaveChat = async () => {
    ...
    try {
      ...
      if (historyId) {
        ...
        const shouldNavigate = window.confirm("상담 내용이 저장되었습니다. 상담 히스토리 페이지로 이동하시겠습니까?")
        if (shouldNavigate) {
  ...
  return (
    <Card className="w-full h-[700px] max-h-[80vh] flex flex-col">
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          육아 관련 질문에 전문적인 답변을 제공합니다. 의학적 진단이나 응급 상황은 전문의와 상담하세요.
        </CardDescription>
        <div className="mt-2">
          <Tabs value={category} onValueChange={(value) => setCategory(value as CategoryType)} className="w-full">
