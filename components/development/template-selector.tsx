...
import { getTemplatesByAgeAndArea, type DevelopmentTemplate } from "@/lib/development-templates"
...
export function TemplateSelector({ ageGroup, developmentArea, onSelectTemplate }: TemplateSelectorProps) {
  const [templates, setTemplates] = useState<DevelopmentTemplate[]>([])
  const [isOpen, setIsOpen] = useState(false)

  useEffect(() => {
    if (ageGroup && developmentArea) {
      const filteredTemplates = getTemplatesByAgeAndArea(ageGroup, developmentArea)
  ...
  if (templates.length === 0) {
    return <div className="text-sm text-muted-foreground mt-2">선택한 연령과 발달 영역에 대한 템플릿이 없습니다.</div>
  }
      ...
      {isOpen && (
        <Card className="mt-2 border border-input">
          <CardHeader className="pb-2">
            ...
            <CardDescription>연령과 발달 영역에 맞는 템플릿을 선택하세요</CardDescription>
          </CardHeader>
          <CardContent>
